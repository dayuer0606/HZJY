...
/**
 * Created by ccy on 2019/8/17/11:30
 */
 ...
 */
public class CountDownTextView extends RoundTextView implements LifecycleObserver, View.OnClickListener {
    ...
    private int mCuntDownTimecolor = -1;
    private int frontEnd = 0;
    private Map<String, Integer> mPartColorHashMap;
    ...
    public CountDownTextView setCountDownText(String front, String latter) {
        frontEnd = TextUtils.isEmpty(front) ? 0 : front.length();
        mCountDownText = front + "%1$s" + latter;
        return this;
    }

    public void setTimeColor(@ColorInt int cuntDownTimecolor) {
        this.mCuntDownTimecolor = cuntDownTimecolor;
    }

    /**
     * 设置某些字段的颜色
     *
     * @param PartColorHashMap
     */
    public void setPartColorOfText(Map<String, Integer> PartColorHashMap) {
        this.mPartColorHashMap = PartColorHashMap;
    }
    ...
    private void count(final long time, final long offset, final TimeUnit timeUnit, final boolean isCountDown) {
        ...
        mCountDownTimer = new CountDownTimer(millisInFuture, interval) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                String text = String.format(mCountDownText, showTime);
                SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder(text);
                if (mCuntDownTimecolor != -1) {
                    ForegroundColorSpan mForegroundColorSpan = new ForegroundColorSpan(mCuntDownTimecolor);
                    spannableStringBuilder.setSpan(mForegroundColorSpan, frontEnd, frontEnd + showTime.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
                }
                if (mPartColorHashMap != null && !mPartColorHashMap.isEmpty()) {
                    for (Iterator iterator = mPartColorHashMap.entrySet().iterator(); iterator.hasNext(); ) {
                        Map.Entry<String, Integer> entry = (Map.Entry<String, Integer>) iterator.next();
                        String key = entry.getKey();
                        int del = text.indexOf(key, -1);
                        if (del != -1) {
                            ForegroundColorSpan mForegroundColorSpan = new ForegroundColorSpan(entry.getValue());
                            spannableStringBuilder.setSpan(mForegroundColorSpan, del, del + key.length(), Spanned.SPAN_INCLUSIVE_EXCLUSIVE);
                        }
                    }
                }
    ...
    public void setCountdownTime(int count) {
        long l = mIntervalUnit.convert(count, TimeUnit.MILLISECONDS);
        String showTime;
        if (mShowFormatTime) {
            showTime = generateTime(count);
        } else {
